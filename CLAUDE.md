...
- **shadcn/ui on Base UI** (`base-nova` preset) — components land in `components/ui/` (currently `button`, `select`), `cn` helper in `lib/utils.ts`. Add more via `pnpm dlx shadcn@latest add <name>` — the auto-picked variant matches the preset. **Base UI Select gotchas** (learned the hard way): `onValueChange` fires with `string | null` (not just `string`), and empty-string values are reserved for the "no selection" state — items that need to represent "unset / none / auto" use a sentinel constant that maps back to `""` at the boundary. See `AUTO_SENTINEL` in `source-uploader.tsx`, `NONE_SENTINEL` + `UNMAPPED_SENTINEL` in `mapping-editor.tsx` for the pattern
