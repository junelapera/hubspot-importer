...
- `proxy.ts` — Auth gate on Edge (renamed from `middleware.ts` for Next 16 per the framework's naming migration). **Two auto-selected modes:** (1) if `BASIC_AUTH_USER` + `BASIC_AUTH_PASSWORD` set → legacy shared-credential Basic Auth (browser prompt, constant-time compare); (2) otherwise → Supabase session check via `@supabase/ssr` `createServerClient` + redirect to `/login?next=<url>` if unauthenticated. Both env pairs unset → dev-mode fallback, no auth. Public paths (`/login`, `/register`, `/api/auth/*`, `/favicon*`, static assets) always pass through. **Uses `NextResponse.next({ request: req })` + cookie writeback pattern** — Supabase rotates access tokens on read within a refresh window, and the refreshed cookie needs to land on the response or the next request uses the stale token. Setting only one Basic Auth env var fails closed
